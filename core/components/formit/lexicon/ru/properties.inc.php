...
$_lang['prop_formit.trimvaluesdeforevalidation_desc']           = 'Обрезать ли пробелы от начала и конца значений до начала валидации. По умолчанию true.';
...
$_lang['prop_formit.emailreturnpath_desc']                      = 'Необязательный. Если `email` установлен как хук, и этот параметр включен,то будет указан Return-path: адрес для электронной почты. Если не установлен, будет принимать значение свойства `emailFrom`.';
...
$_lang['prop_formit.emailmultiseparator_desc']                  = 'Разделитель по умолчанию для списка значений, отправляемых через поля типа checkbox / multi-select. По умолчанию используется перенос строки.';
...
$_lang['prop_fico.allgrouptext_desc']                           = 'Необязательный параметр. Если установлено и используется &prioritized,то будет использоваться метка (label) для всех других стран групп опций.';
